...
def Check(proxy, limit):
    ...
    while 1:
        ...
        if temp_pool == now_pool:
            if time.time() - temp_time > 1200:
                return res
        else:
            temp_time = now_time
            
        if pool == 0:# or now_time > 7200:#5400:
            return res
...
def getOldTime(name):
    f = open(name, "r")
    oldtime = float(f.read())
    f.close()
    return oldtime

def saveOldTime(name, nowAtime):
    f = open(name, "w+")
    f.write(str(nowAtime))
    f.close()
    return 1
...
try:
    getOldTime("OldTime.tmp")
except:
    saveOldTime("OldTime.tmp", time.time())
...
#    if len(result) < CHECK_LIMIT:
    jobs = [(job_server.submit(func, (), (getHtml, getRe, Out, Kuaidaili), ("random", "requests", "ssl", "re"))) for func in funcs]
    for getProxy in jobs:
        try:
            TEMP = getProxy()
            for k in TEMP:
    ...
    if (time.time() - getOldTime("OldTime.tmp")) > CHECK_LIMIT:
        results = []
        saveOldTime("OldTime.tmp", time.time())
        for k in range(HowManyTimes):
            Out("Wr", "The {}th times.".format(k))
            Tr = Check(result, 50)
            Out("Wr", "Saving to result..")
            for ps in Tr:
                if not(ps in results):
                    results += [ps]
            Out("Ok", "Done, now there have {}.".format(len(results)))
        result = results
        delay = 240
    else:
        delay = 120
